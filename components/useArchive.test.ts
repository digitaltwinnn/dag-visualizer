...
import { archiveValue, fmtReach, fmtSnapCount, type ArchiveEntry } from "./useArchive";
...
describe("archive value", () => {
  ...
  it("window value carries both registers; genesis and deep carry their claims", () => {
    const now = Date.now();
    ...
    expect(archiveValue(win, "Nov 2023")).toBe("Last ~3 months · 241k snapshots");
    expect(archiveValue({ ...base, kind: "genesis" }, "Nov 2023")).toBe("From genesis");
    expect(archiveValue({ ...base, kind: "deep" }, "Nov 2023")).toBe("Back to Nov 2023");
  });
  ...
  it("falls back to count alone when the floor date is unknown", () => {
    ...
    expect(archiveValue(win, "Nov 2023")).toBe("Last ~5k snapshots");
  });
});
