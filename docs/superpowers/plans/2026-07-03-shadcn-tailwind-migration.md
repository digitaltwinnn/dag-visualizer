...
- **Acceptance bar (user-calibrated 2026-07-04):** the HUD's custom Instrument-Glass character is preserved — never remove or genericize a bespoke design element (glass gradient, spine, thread, ECG, glyphs, odometer, states); the app must keep its own character, not become a run-of-the-mill site. Within that, **use the framework idiomatically**: prefer tokens, theme utilities, and shadcn/Radix primitives as they are meant to be used over contorted pixel-exact arbitrary values — slight minor rendering deltas are acceptable. The **design system must be 100% consistent** (two lanes, tokens, one conversion idiom). Visual checking happens twice — Task 1 baseline, Task 10 comparison (judged for character/element preservation, not pixel equality).
- **Design-concern log:** any UI/UX concern noticed while migrating (odd spacing, inconsistent affordance, questionable interaction) is NOTED in `docs/superpowers/plans/2026-07-04-migration-design-concerns.md` — never fixed inline. A design/brainstorming session reviews the list after the migration.
...
- [ ] **Step 4b: Unify the card header (user-requested 2026-07-04).** Every HUD card shares one visual grammar — uppercase role eyebrow ("SELECTED METAGRAPH" / breadcrumb "SNAPSHOT ‹ DOR" / "HYPERGRAPH · ABOUT"), a title that rolls in on subject change (`roll-in` keyed remount), a right-side action (× clear or +/– collapse) — but three implementations exist (`PanelHead`, `InspectorCard`'s own eyebrow duplicating `ContextCard`'s literal, hand-rolled close buttons in `ContextCard`/`Inspector` CardPane). Build ONE shared header primitive (extend `PanelHead` or a new `CardHead` in `components/`) with: eyebrow slot (plain / identity-dot / breadcrumb), keyed roll-in title slot, right-action slot (collapse or close with its aria/title semantics preserved). Consume it from `PanelHead` call sites, `InspectorCard`, `ContextCard`, and `Inspector` CardPane. Rendered output unchanged.
