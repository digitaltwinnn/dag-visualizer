...
- **Overlay primitive:** shadcn **Sheet** (add `components/ui/sheet.tsx` on radix Dialog). **NON-modal** — either OR both rails may be open at once (USER DECISION: Explore drives Details, so the user chooses), and the scene between them stays interactive (picking works). No blocking scrim. Each sheet is dismissed independently by its own ✕ (or Escape). The sheet container is a LIGHT glass (more see-through than `--panel`) so the scene reads behind it; cards keep their fuller glass; the side sheets carry the cyan spine + hairline ruler on their screen-edge side.
...
- **Phone rail affordance (USER DECISION):** on phone, a PERSISTENT narrow split bottom BAR (always visible, semi-transparent light glass like the tablet sheets) is anchored to the REAL bottom of the screen — BELOW the LiveStrip bar-chart (raise `#livestrip`'s bottom offset on phone so it sits just above the bar, no overlap). The bar has two equal halves with an icon+label each: LEFT "Explore", RIGHT "Details" (Details carries the new-detail hint). Tapping a half slides the section UP to reveal that rail's scrollable cards (~60vh, scene visible above); the bar stays as the header so it reads as the bar expanding upward. ONE AT A TIME (tapping the other half switches; tapping the active half toggles closed) via the `phoneDock` store field. Non-modal, no auto-open (a scene pick only pulses the Details hint). Tablet keeps the side edge tabs + side sheets (both-open); desktop unchanged.
...
- [ ] **Step 0: Remove the left legend so the bars span the full width (USER DECISION — all sizes).** Drop the `.ls-live` legend block (the live/no-signal dot + "Global L0" + the `.ls-scale` "anchors/tick" text) from `LiveStrip.tsx` so the `.ls-bars` bar-chart spans the entire strip width. The bars are self-evident in context (the axis ruler + the app). Preserve the NO-SIGNAL state cue — `#livestrip.no-signal` already desaturates the whole strip (Phase 7), so the state is still legible without the dot; keep that class wiring. Remove the now-unused `.ls-live`/`.ls-scale` CSS (and the `live`/`ns-dot` markup) — but do NOT touch the tooltip (`#ls-tip`), the bars, or the axis ruler.
- [ ] **Step 0b: LiveStrip always present (USER DECISION).** `components/BottomStream.tsx` currently hides the strip in the placeholder views (`status`/`transactions`/`staking`) and zeroes `--bottom-reserve`. It's the network's global live heartbeat, view-independent — always render `<LiveStrip />` and always set `--bottom-reserve: 130px`, in EVERY view including the "soon" placeholders. (Note: this supersedes the CLAUDE.md line that says the placeholder views have an empty bottom stream — update that doc in the final pass.)
